...
function GameCard({ game }: { game: Game }) {
  ...
  return (
    <div className="group relative bg-[#1d2022]/40 backdrop-blur-md border border-neon-yellow/20 hover:border-neon-yellow/60 transition-all duration-300 p-4">
      ...
      <div className="relative overflow-hidden aspect-video mb-4 border border-neon-yellow/10">
        ...
        <div className="absolute inset-0 bg-neon-yellow/5 group-hover:bg-transparent transition-all duration-300" />
      </div>
      <h3 className="text-lg font-bold text-neon-yellow mb-1">{game.title}</h3>
      <p className="text-[11px] text-zinc-400 mb-4 leading-relaxed">{game.description}</p>
      <Link
        href={game.link}
        target={isExternal ? "_blank" : "_self"}
        rel={isExternal ? "noopener noreferrer" : ""}
        className="w-full flex items-center justify-center gap-2 py-2 border border-neon-yellow/40 text-neon-yellow text-xs font-semibold tracking-wider hover:bg-neon-yellow hover:text-black transition-colors"
        style={{
          clipPath:
        ...
        <div className="pt-6 mb-8 flex flex-col md:flex-row justify-between items-start md:items-end gap-4 shrink-0">
          <div className="flex gap-4">
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-3"><div className="w-2 h-2 bg-neon-yellow/40" /><span className="font-orbitron text-[0.7rem] text-neon-yellow/60 tracking-[0.4em] font-black uppercase">MODULE_05: ENTERTAINMENT_MODULE</span></div>
              <h1 className="font-scary text-[#CCFF00] text-4xl md:text-7xl lg:text-8xl tracking-tight leading-[0.9] text-glow-yellow uppercase italic">GAMES<span className="text-white">.</span></h1>
            </div>
          </div>
